...
battery cycler, provided it has the necessary data sampling frequency (data is recorded every 10 ms during current interruption). To the ensure that electrochemical processes within the cell are not kinetically limited, it is recommended that the cell is cycled at a slow cycling, e.g., C/5 or slower. Therefore, a single experiment can contain hundreds of interruptions spread over many cycles. Each pulse requires pulse segmentation, selection
